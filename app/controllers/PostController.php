...
class PostController extends \BaseController {
	...
	public function rss() {
		try {
			return Cache::section('posts')->rememberForever('rss', function()
				{
	...
	public function index()
	{
		...
		foreach($posts as $post) {
			$view = (!Auth::check()) ? Cache::section('posts')->rememberForever('view_'.$post->id, function() use($post)
			{
		...
		return View::make('posts/posts')->with('post_views', $post_views);
	}
	...
	public function offset($offset = 0)
	{
		$limit = $this->limit;
		$offset_func = '_offset';
		$post_views = (!Auth::check()) ? Cache::section('offsets')
										->rememberForever($offset, function() use($offset, $offset_func, $limit) {
											return $offset_func($offset, $limit);
										})
										: $offset_func($offset, $limit);

		return Response::json(array('count' => sizeof($post_views), 'html' => $post_views));
	}
	...
	public function show($id)
	{
		...
		$view = '';
		if (!Auth::check()) {
			$view = Cache::section('posts')->rememberForever('view_'.$id, function() use($id)
				{
	...
	public function store()
	{
		...
		Cache::section('posts')->forget('rss');
